...
df_forecast_p2 %>%
  ...
  annotate(geom = "text", x = df_target_achieved_p2$date+3, y = df_target_achieved_p2$target+4, label = glue("(Dt: {df_target_achieved_p2$date}, Wt: {df_target_achieved_p2$target} lb)"), size=7, hjust = "left") +
  scale_color_tableau() + 
  scale_fill_tableau() + 
  theme_fivethirtyeight() +
