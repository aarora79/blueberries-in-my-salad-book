...
df_forecast_p2 %>%
  ...
  annotate(
    geom = "curve", xend = df_target_achieved_p2$date, yend = df_target_achieved_p2$target, x = df_target_achieved_p2$date+2, y = df_target_achieved_p2$target+4, 
    curvature = .3, arrow = arrow(length = unit(2, "mm"))
  ) +
  annotate(geom = "text", x = df_target_achieved_p2$date+3, y = df_target_achieved_p2$target+4, label = glue("(Dt: {df_target_achieved_p2$date}, Wt: {df_target_achieved_p2$target} lb)"), hjust = "left") +
  scale_color_tableau() + 
  scale_fill_tableau() + 
  theme_fivethirtyeight() +
  ...
  labs(title=glue("When will Amit achieve his target weight?"),
       subtitle=glue("Light blue lines show the error range for the forecast. Forecast done on {FORECAST_DATE}.")) +
  theme(text = element_text(size=CHART_ELEMENT_TEXT_SIZE-5), legend.title = element_blank()) +
...
df_forecast_p1 %>%
  ...
  annotate(geom = "text", x = df_target_achieved_p1$date+2, y = df_target_achieved_p1$target+2, label = glue("(Dt: {df_target_achieved_p1$date}, Wt: {df_target_achieved_p1$target} lb)"), hjust = "left") +
  scale_color_tableau() + 
  scale_fill_tableau() + 
  theme_fivethirtyeight() +
  scale_x_date(date_breaks = "3 month", date_labels = "%b %Y") +
