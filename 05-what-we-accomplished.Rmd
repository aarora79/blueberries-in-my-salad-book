...
df_forecast_p2 %>%
  ...
  annotate(geom = "text", x = df_target_achieved_p2$date-8, y = df_target_achieved_p2$target+7, label = glue("(Dt: {df_target_achieved_p2$date}, Wt: {df_target_achieved_p2$target} lb)"), size=7, hjust = "left") +
  scale_color_tableau() + 
  scale_fill_tableau() + 
  theme_fivethirtyeight() +
...
df_forecast_p1 %>%
  ...
  annotate(
    geom = "curve", xend = df_target_achieved_p1$date, yend = df_target_achieved_p1$target+1, x = df_target_achieved_p1$date+2, y = df_target_achieved_p1$target+2, 
    curvature = .3, arrow = arrow(length = unit(2, "mm"))
  ) +
