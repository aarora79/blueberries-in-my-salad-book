...
description: "This book describes a journey of clean eating and regular workouts that I undertook with my wife from being fat to fit. Like a salad, life may not be the most appealing lunch option but if you can keep finding the blueberries every now and again, it is both rewarding and fullfilling."
classoption: oneside
...
flog.info(glue("params used:
                ...
                P1_NAME={P1_NAME},
                P2_NAME={P2_NAME},
                P1_DATA_URL={P1_DATA_URL},
                P2_DATA_URL={P2_DATA_URL},
                CAPTION={CAPTION},
...
flog.info(glue("read data for {P1_NAME} from {P1_DATA_URL}, shape of dataframe={nrow(df_P1)}x{ncol(df_P1)}"), name=LOGGER)
...
if(!is.na(P2_NAME)) {
  df_P2 <- read_csv(P2_DATA_URL) %>%
    ...
    mutate(Date=ymd(Date)) %>%
    filter(Date >= START_DATE)
  flog.info(glue("read data for {P2_NAME} from {P2_DATA_URL}, shape of dataframe={nrow(df_P2)}x{ncol(df_P2)}"), name=LOGGER)
...
if(!is.na(IMPORTANT_DATES_FPATH)) {
  ...
  flog.info(glue("read data for important dates from {IMPORTANT_DATES_FPATH},
